...
import { formatToUSDTPrice, formatToFACPrice, formatToUSDPrice } from "../../../helper/common"
...
function ExchangeFAC() {
  ...
  function rechargeHistory(filter) {
    ...
    PaymentExchangeTransaction.userExchangeFACHistory(Object.assign(newFilter, dateFilter)).then((result) => {
  ...
  const columns = [
    ...
    {
      title: t('FAC_price_transaction'),
      dataIndex: 'exchangeRate',
      key: 'facPrice',
      render: (text, row, index) => {
        const { exchangeRate } = row
        return <div className="d-flex align-items-center">
          <span className="management__box__coin recharge_history__normal_text">{formatToUSDPrice(exchangeRate, false, true)}</span>
        </div>
      }
    },
    {
      title: t('exchange_amount_transaction'),
    ...
    {
      title: t('receive_amount_transaction'),
      ...
      render: (text, row, index) => {
        ...
        return <div className="d-flex align-items-center">
          <span className="management__box__coin recharge_history__normal_text">
            {formatToUSDTPrice(Math.abs(receiveWalletBalanceBefore - receiveWalletBalanceAfter), false)}
          </span>
        </div>
      }
    },
    ...
    <section className="management recharge_history">
      <div className="management__box">
        <div className="recharge_history__header">
          <div className="management__box__detail">{t('exchange_fac_history')}</div>
